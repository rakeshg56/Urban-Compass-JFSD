...
<body>
  ...
  <script>
    ...
    // Convert JSTL data into JavaScript objects
    const hotelData = [
      <c:forEach items="${hlist}" var="h">
        { name: '${h.name}', email: '${h.email}', hotel: '${h.hotel}', checkindate: '${h.checkindate}', checkintime: '${h.checkintime}', checkoutdate: '${h.checkoutdate}', checkouttime: '${h.checkouttime}' },
      </c:forEach>
    ];

    const rentalData = [
      <c:forEach items="${rlist}" var="r">
        { name: '${r.name}', contact: '${r.number}', email: '${r.email}', address: '${r.address}', vehicle: '${r.vehicle}', date: '${r.date}', time: '${r.time}' },
      </c:forEach>
    ];
    ...
    const hotelCounts = countOccurrences(hotelData, 'hotel');
    const hotelLabels = Object.keys(hotelCounts);
    const hotelValues = Object.values(hotelCounts);
    ...
    new Chart(document.getElementById('hotelChart'), {
      ...
      data: {
        ...
        datasets: [{
          ...
          data: hotelValues,
          backgroundColor: ['#FF6384', '#36A2EB', '#FFCE56', '#8F4700'],
        }]
      }
    });

    // Rental bookings chart
    const vehicleCounts = countOccurrences(rentalData, 'vehicle');
    const vehicleLabels = Object.keys(vehicleCounts);
    const vehicleValues = Object.values(vehicleCounts);
